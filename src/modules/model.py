...
def build_styler(PARAMS):
    ...
    S.add(layers.Dense(512, input_shape = [PARAMS['latent_dim']]))
    S.add(layers.LeakyReLU(0.2))
    S.add(layers.Dense(512))
    S.add(layers.LeakyReLU(0.2))
    S.add(layers.Dense(512))
    S.add(layers.LeakyReLU(0.2))
    S.add(layers.Dense(512))
...
def build_synthesis(PARAMS):
    ...
    n_layers = int(np.log(PARAMS['data_dim']) / np.log(2)) - 1

    input_style = []
    for i in range(n_layers):
        input_style.append(layers.Input([512]))

    input_noise = layers.Input([PARAMS['data_dim'], PARAMS['data_dim'], 1])

    y = layers.Input((PARAMS['attr_num']))

    channel = 48
    # Actual Model
    x = layers.Dense(4*4*4*channel, activation = 'relu', kernel_initializer = 'he_normal')(y)
    x = layers.Reshape([4, 4, 4*channel])(x)
    x = gen_block(x, input_style[0], input_noise, 16 * channel, up = False)  
    x = gen_block(x, input_style[1], input_noise, 8 * channel)  
    x = gen_block(x, input_style[2], input_noise, 6 * channel)  
    x = gen_block(x, input_style[3], input_noise, 4 * channel)  
    x = gen_block(x, input_style[4], input_noise, 3 * channel)  
...
def build_image_discriminator(PARAMS):
    ...
    h = layers.Dense(1, kernel_initializer = 'he_normal')(h)
...
def build_z_discriminator(PARAMS):
    ...
    h = layers.Dense(256, kernel_initializer = 'he_normal')(x)
    h = layers.LeakyReLU(0.2)(h)
    h = layers.Dense(128, kernel_initializer = 'he_normal')(h)
    h = layers.LeakyReLU(0.2)(h)
