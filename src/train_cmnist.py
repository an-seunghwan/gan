...
from modules import model_cmnist
#%%
PARAMS = {
    "batch_size": 10,
    "epochs": 10000, 
...
if PARAMS['data'] == "cmnist":
    ...
    def colored_mnist(image):
        ...
        # size
        size = np.random.uniform(0.25, 0.5, 1)
    ...
    # plt.savefig('./CIFAR10_assets/sameclass.png',
    ...
    assert 0 == 1
#%%
encoder = model_cmnist.build_encoder(PARAMS)
encoder.summary()
generator = model_cmnist.build_generator(PARAMS)
generator.summary()
img_discriminator = model_cmnist.build_image_discriminator(PARAMS)
img_discriminator.summary()
z_discriminator = model_cmnist.build_z_discriminator(PARAMS)
z_discriminator.summary()
classifier = model_cmnist.build_image_classifier(PARAMS)
classifier.summary()
#%%
@tf.function
def loss_function(x_batch, y_batch, PARAMS):
    ...
    # 5. classifier
    classification_loss1 = tf.reduce_mean(tf.math.log(tf.reduce_sum(classifier(x_batch) * y_batch, axis=-1)))
    classification_loss2 = tf.reduce_mean(tf.math.log(tf.reduce_sum(classifier(reconstructed_images) * y_batch, axis=-1)))
...
def train_one_step(x_batch, y_batch, PARAMS):
    ...
    gradients_of_classifier.apply_gradients(zip(gradients_of_classifier, classifier.trainable_variables))
...
for _ in progress_bar:
    ...
    progress_bar.set_description('setting: {} | iteration {}/{} | enc loss {:.3f}, gen loss {:.3f}, img loss {:.3f}, z loss {:.3f}, cls loss {:.3f}'.format(
        PARAMS['data'], 
        step, PARAMS['epochs'], encoder_loss, generator_loss, img_dis_loss, z_dis_loss, classification_loss
    ))
